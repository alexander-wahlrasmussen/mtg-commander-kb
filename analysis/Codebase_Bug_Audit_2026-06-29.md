...
- 🔴 **`ct_speed_lab.py` dig** — the big one: models Glarb's SELECTION (surveil/look-at-top) as raw card DRAW (+2/turn), inflating the Croak clock. The published Croak decap ~T10 / table ~T10 is ~3 turns too fast (honest dig=0 → decap ~T13 / table never-in-horizon). Needs: fix → re-bake the croak harvest entry + recompute P(beat pod) + tier-list + dashboard + Croak Summary re-derivation.
