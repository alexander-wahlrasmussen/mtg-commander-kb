...
def test_kill_tree_every_encoded_deck_resolves():
    ...
    a title/root/stall/src, and (if present) a valid background kind. All 15 active decks
    are encoded (was 17; Diminishing Returns retired 2026-07-08, Earthbend the Meta 2026-07-11);
    assert the registry and the resolver agree."""
    specs = kb.deck_registry.KILL_TREES
    encoded = [sp["reg_slug"] for sp in specs.values()]
    assert len(encoded) == len(specs) == 15
