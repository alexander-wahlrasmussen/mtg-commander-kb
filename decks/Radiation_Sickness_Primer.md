# Radiation Sickness — Pilot Primer

**Commander:** The Wise Mothman (Sultai / BUG)
**Archetype:** Proliferate / rad counters / +1/+1 counters · **Bracket:** 3
**Full detail:** see `Radiation_Sickness_Summary.md`. This page is for piloting at the table.

---

## The pitch

Two interlocking counter systems. Mothman hands a **rad counter** to every player on ETB and attack — those force each opponent to mill and lose life every turn. **Proliferate** then amplifies *both* the opponents' rad counters *and* the +1/+1 counters on your creatures at once. Every mill event feeds Mothman to drop more +1/+1 counters; proliferate doubles those again. Spiral the counters into a lethal board, a Simic Ascendancy win, a Triumph infect swing, or a Mindcrank combo.

## How you win

- **Counter overload combat (primary):** proliferate + Mothman + doublers grow 15+ power creatures; **Champion of Lambholt** makes the team unblockable, **Kodama of the West Tree** grants trample. ~3–4 proliferate cycles with Tekuthal out.
- **Triumph of the Hordes (single-card swing):** team gets +1/+1, trample, and **infect** — 10 poison is lethal regardless of life, off a wide board.
- **Simic Ascendancy (alt-win):** 20 growth counters → you win; doublers + proliferate hit the Ascendancy directly, ~2 turns from deployment.
- **Mindcrank + Bloodchief Ascension (2-card infinite):** once Bloodchief has 3 quest counters, any life loss → mill → more life loss → kills the table. Flag to the pod.
- **Backups:** Toxrill attrition (non-combo, non-combat); Walking Ballista counter sink; Hornbeetle / Herd Baloth token floods.

## Mulligan — what to keep

Looking for: **lands + ramp + Mothman or a proliferate engine.**

- **Keep:** ramp + Mothman + a proliferate source or a counter doubler.
- **Toss:** no-land hands; all-payoff with no engine to grow counters.

## Turn-by-turn

1. **T1–3:** ramp (Sol Ring, dorks, Farseek, Nature's Lore).
2. **T4:** Mothman — rad counters start hitting all players.
3. **T5+:** proliferate engines + doublers (Tekuthal, Doubling Season, Vorinclex MR). Counters spiral; opponents mill and bleed more each turn.
4. **Closing turn:** combat overload, Triumph infect, a Simic Ascendancy tick to 20, or assemble Mindcrank + Bloodchief.

## Don't-miss rulings

- **Rad counters decay ONLY when *nonland* cards are milled** — milling a land removes no rad counter, so it persists into the next turn. Fighting that decay with proliferate is the whole engine.
- **Rad counters hit EVERYONE, including you** — you self-mill and lose life. Morgul-Knife (Shadowspear) lifelink is the built-in offset.
- **Proliferate adds rad counters to opponents AND +1/+1 counters to your creatures simultaneously** — the central interaction.
- **The multipliers stack multiplicatively:** Tekuthal doubles all proliferate; **Vorinclex MR** doubles every counter you place on opponents (Mothman ETB → 2 rad each) *and* your own placements; Doubling Season doubles counters on your permanents — including Bloodchief's quest counters, so the combo arms in 2 life-loss triggers, not 3.
- **Mothman's +1/+1 distribution triggers once per mill *event***, regardless of how many players milled at once.
- **Mindcrank + Bloodchief is not online the turn Bloodchief lands** — it needs 3 quest counters first (proliferate accelerates this).

## Threats & timing

- **How you lose:** **enchantment/artifact removal.** Doubling Season, Hardened Scales, Branching Evolution, Simic Ascendancy, and Bloodchief Ascension are all targets; recovery is thin (Regrowth only).
- **Also incidental graveyard hate** — opponents' yards fill from your rad counters; your own Bojuka Bog polices theirs.
- **Interaction:** 12 pieces with one free counter (Force of Negation, works without commander on the field); Heroic Intervention protects the board from a wipe.
- **Goldfish T6–9.** Mothman is a modest 3/3 flyer that survives early, but experienced players will recognize the proliferate engine and target it — protect Tekuthal / Doubling Season / Seedborn Muse.

## Reskins (for borrowers)

| On the card | Really is | What it does |
|---|---|---|
| Morgul-Knife | Shadowspear | {1} equip; +1/+1, trample, lifelink (offsets your rad self-damage); {1}: opponents' permanents lose hexproof and indestructible until end of turn. |
