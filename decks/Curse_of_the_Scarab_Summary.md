...
**Post-wipe resilience — measured, not narrated (`cos_clock_lab.py --mode recover` @8k, 2026-07-03).** A creature board wipe costs the deck **~2–3 turns** — it does *not* bounce off. Table clock T11→T13–14, decap T8→T11 — but it *does* grind back and still closes in the median. The engine of the comeback is the **enchantment/planeswalker core** (Kindred Discovery, Necroduality, Rooftop Storm, the two Lilianas) plus mana rocks — all survive a creature wipe and redeploy bodies — so the deck rebuilds *faster the later it is wiped* (median recovery to a threatening board +4 turns if swept at T6, +2 if at T10, as more engine and graveyard fuel is already down). **Mikaeus caveat:** undying only saves **nontoken** creatures, and this go-wide board is mostly tokens (which can't return via undying and don't stock the yard), so "two wipes to clear" held in only ~0–2% of swept games — the token swarm is genuinely gone after one wipe. Net: **resilient, not wipe-proof.** The absolute self-clock tax is real (+2–3 turns); the goldfish can't see the *relative* pod upside — your persistent engines recover faster than a normal creature board, so a symmetric wipe can still favour you positionally even while it taxes your clock.
...
**Through interaction: T9–12.** A board wipe on turn 6–7 resets zombie count. Recovery costs **~2–3 turns** (measured, `--mode recover`; not the 1–2 once claimed — a wipe pushes the table clock to ~T13–14), rebuilt off the surviving enchantment engines + reanimation; Demonic Tutor finds mass reanimation; Living Death, Agadeem's Awakening, Rot Hulk's ETB, and Liliana DM's -3 are four distinct reanimation axes. Fierce Guardianship and Force of Negation protect key turns without mana investment. Cyclonic Rift creates lethal alpha strikes that bypass normal combat math. Games against interaction-heavy pods close by turn 9–12.
...
1. **Grinds through attrition.** Board wipes *tax* this deck ~2–3 turns (measured, `--mode recover`) but rarely end it — the enchantment engine core survives and reloads bodies from a graveyard the wipe just stocked. They are a real setback for you, not a free help; the relative upside is that your persistent engines recover faster than the opponent's cleared board, so repeated wiping trades their cards for your (replaceable) tokens and buys the pod time. Mikaeus only shields your *nontoken* creatures — the token swarm is gone after one wipe, so don't over-rely on "two wipes to clear."
...
- **Board wipes cost you ~2–3 turns but you grind back** (measured — see Durability §). They stock the yard for Living Death / Agadeem's / Rot Hulk and your enchantment engines survive to redeploy, so don't play *scared* of them — but they're a real setback, not a help, and Mikaeus only saves your *nontoken* bodies (the token swarm is gone). Rebuild off the surviving engines + reanimation, a couple turns behind.
