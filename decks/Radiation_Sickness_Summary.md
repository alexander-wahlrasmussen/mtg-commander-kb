...
**Upgrade history:**
- **2026-07-07 — Mana-dork swap: −Birds of Paradise +Nightshade Dryad ($0, owned ×3).** Birds was pulled into **Zero-Sum Game** on physical sleeving; Nightshade Dryad ({1}{G}, taps any color, deathtouch) preserves the Sultai fixing one turn slower, so the swap is **clock/GC/CC-neutral** (neither card is a GC). Current list `radiation-sickness-20260707.txt` (`-20260622` archived); deck_doctor 100 cards / 3-GC ✓.
