...
DECKS = {
    ...
    "bumbleflower": {
        ...
        "cc": 16, "cc_axes": (4, 4, 3, 5),
        "win_line": {"pieces": ["Jolrael, Mwonvuli Recluse", "Approach of the Second Sun"], "fuzzy": True,
                     "line": "Jolrael alpha / Approach 2nd-cast / Alarm-loop — plural quiet exits (2026-07-03)"},
        "bottleneck": "BOARD", "min_lands": 2, "max_lands": 4, "hi_curve": False, "mixed": None,
    },
    "eldrazi_stampede": {
...
KILL_TREES = {
    ...
    "bumbleflower": {
        ...
        "background": None,
        "lines": [
            ("alarm", "Intruder Alarm + Shrieking Drake / Whitemane Lion + any repeatable dork<br/>+ the commander",
             "each self-bounce recast = a Bumbleflower trigger (mana-neutral) → \"target opponent<br/>draws\" iterated: the table draws out on the spot (Lion = instant speed)", "table on assembly (8% of closes)", "combo"),
            ("approach", "Approach of the Second Sun — 2nd cast from HAND = win<br/>(Reprieve bounces your own 1st cast back to hand: win checks CAST, not resolved)",
             "cast #1 at 7 mana → redraw it (7th from top; the engine draws 2–4/turn)<br/>or Reprieve-split (9 mana, then 7) — protected by the counter suite", "table T9–10 (32% of closes)", "combo"),
            ...
            ("wizfathom", "Wizard Class lvl 3 + Fathom Mage",
             "every draw → counter on Fathom → may draw → draw the library at will<br/>(find an exit; Jolrael X = huge)", "enabler", "enabler"),
            ("counters", "pile +1/+1 counters & flying on one carrier each cast (fallback)",
             "a single evasive counter-grown threat flies over", "decap T8+ (slowest)", "combat"),
        ],
        "stall": "out-value the table — chain cantrips for the 2nd-cast draw-two, deploy draw payoffs,<br/>hold up the deep counter/removal suite; close from dominance via whichever quiet exit assembles<br/>(mixture 60% combat / 32% Approach / 8% Alarm — lab 2026-07-03)",
